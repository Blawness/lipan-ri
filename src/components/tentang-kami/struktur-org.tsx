        ...
        <div className="space-y-0">
          {levels.map((level) => (
            <div key={level} className="relative pl-10 pb-6">
              <div className="absolute left-1.5 top-2 w-5 h-5 rounded-full bg-blue-600 border-4 border-blue-100" />
                ...
                {grouped[level].map((item: any, i: number) => (
                  <Card key={i} className="border-blue-100 hover:shadow-sm transition-shadow">
                    <CardContent className="p-3 flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-blue-900">{item.nama}</span>
                    </CardContent>
                  </Card>
                ))}
              </div>
